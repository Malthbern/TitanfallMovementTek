...
public class WallrunRaycast : UdonSharpBehaviour
{
    ...
    void Update()
    {
        ...
        if (Networking.LocalPlayer.IsPlayerGrounded() == false)
        {
            ...
            else if (Physics.Raycast(transform.position, transform.right, RaycastDistance, Bitmask))
            {
                Debug.Log("Right cast hit");
                BaseTFScript.SetProgramVariable("WallDirection", "Right");
            }
            else if (Physics.Raycast(transform.position, (transform.right + LeftOffset), RaycastDistance, Bitmask))
            {
                Debug.Log("Left cast hit");
                BaseTFScript.SetProgramVariable("WallDirection", "Left");
            }
            else if (Physics.Raycast(transform.position, Vector3.forward, RaycastDistance, Bitmask))
            {
